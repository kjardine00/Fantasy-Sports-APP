  ...
  const renderCommisionerContent = () => {
      ...
      return (
        <>
          <h2 className="text-xl font-bold text-error text-center w-full">
            You're league is not full and your draft has not been scheduled.
          </h2>
          <div className="card-actions justify-evenly py-4">
            <button className="btn rounded-full btn-primary flex-1 max-w-64">
              Invite Managers
            </button>
            <button className="btn rounded-full btn-secondary flex-1 max-w-64">
              Schedule Draft
            </button>
          </div>
        </>
          ...
          <div className="flex-1 text-center">
            ...
            <h3 className="card-subtitle">
              {draft?.scheduled_start
                ? new Date(draft.scheduled_start).toLocaleDateString()
                : "Not scheduled"}
            </h3>
          </div>
        </div>
    ...
    if (isLeagueFull && !isDraftScheduled) {
      return (
        <>
          <h2 className="text-xl font-bold text-error text-center w-full">
            Your Draft has not yet been scheduled.
          </h2>
          <div className="card-actions justify-evenly py-4">
            <button className="btn rounded-full btn-primary flex-1 max-w-64">
              Schedule Draft
      ...
      return (
        <div className="flex flex-row items-center justify-between w-full gap-6">
          <div className="flex flex-col items-start justify-center w-[40%] space-y-2">
            ...
            <div className="mt-2">
              <button className="btn btn-sm rounded-full btn-secondary w-full">
                Edit League Settings
              </button>
            </div>
          </div>
