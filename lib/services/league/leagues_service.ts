...
export class LeagueService {
  ...
  private static async generateUniqueShortCode(): Promise<string> {
    ...
    for (let i = 0; i < maxRetries; i++) {
      ...
      const existsResult = await existsByShortCode(shortCode);

      // If there's an error checking, skip this iteration and try again
      if (existsResult.error) {
        console.warn(`Error checking short code existence: ${existsResult.error}`);
        continue;
      }

      // If the short code doesn't exist (existsResult.data === false), we found a unique one
